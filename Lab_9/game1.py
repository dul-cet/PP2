    ...
    while True:
        ...
        while i < len(points) - 1:
            if mode == 'rectangle':
                ...
                drawCircle(screen, points[i], points[i + 1])
            elif mode == 'eraser':
                drawEraser(screen, i, points[i], points[i + 1], radius)
            elif mode == 'square':
                drawSquare(screen, points[i], points[i + 1])
            elif mode == 'right_triangle':
                drawRightTriangle(screen, points[i], points[i + 1])
            elif mode == 'equilateral_triangle':
                drawEquilateralTriangle(screen, points[i], points[i + 1])
            elif mode == 'rhombus':
                drawRhombus(screen, points[i], points[i + 1])
            else:
...
def drawRhombus(screen, start, end):
    ...
    pygame.draw.polygon(screen, (255, 255, 255), [(start[0], mid_y), (mid_x, start[1]), (end[0], mid_y), (mid_x, end[1])])
